...
export class PageFactory {
     ...
     */
    public static getInstance(page = null): PageFactory {
        if (!PageFactory._instance && !!page) 
            PageFactory._instance = new PageFactory(page);
...
async function updatePageFactory(
  ...
  // Generate imports, properties, and getters only for new classes
  ...
  for (const file of selectedFiles) {
    ...
    properties.push(`    private ${propertyName}: ${className} | undefined;`);
    getters.push(`
    /**
    ...
    public get ${className.toLowerCase()}(): ${className} {
        if (!this.${propertyName}) 
            this.${propertyName} = new ${className}(this.page);
...
async function createPageFactoryInstance() {
  ...
  // Add the instance creation
  textToInsert += "const pageFactory = PageFactory.getInstance();";
